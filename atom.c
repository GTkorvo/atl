...
send_get_atom_msg_ptr msg;
{
    ...
    entry = Tcl_CreateHashEntry(&as->value_hash_table,
				 stored->atom, &new);
    if (!new) {
	printf("1. Serious internal error in atom cache.  Duplicate value hash entry.\n");
	thr_mutex_unlock(as->hash_lock);
	exit(1);
    }
    Tcl_SetHashValue(entry, msg);
...
*/
	    ...
	    stored->atom_string = str;
	    stored->atom = string_to_int(buf); //as->next_atom_id++;
	    ...
	    Tcl_SetHashValue(entry, stored);
	    if (!new) {
		fprintf(stderr, "4. Serious internal error in atom server.  Duplicate string hash entry.\n");
		exit(1);
	    }
	    /* enter into value hash table */
	    entry = Tcl_CreateHashEntry(&as->value_hash_table,
	    ...
	    Tcl_SetHashValue(entry, stored);
	    if (!new) {
		printf("5. Serious internal error in atom server.  Duplicate value hash entry.\n");
		exit(1);
	    }
